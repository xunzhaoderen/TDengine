...
class TDTestCase:
    ...
    def createSingleFile(self, stbNum, FileIndex, IP):
        ...
        for i in range(stbNum):
            self.stable_insert(int(i + FileIndex),dict(stbTemplate), localTaosdemoConfig)
    ...
    def run(self):
        ...
        threadlist = []
        for i in range(8):
            threadlist.append(threading.Thread(target = self.creationThread, args = (15,binPath,i,IP1,)))
            threadlist[i].start()
        for i in range(8):
            threadlist[i].join()
