...
class TDTestCase:
    ...
    def checkRegularTableWildcardLength(self):
        ...
        exceed_sql_list = [f'show tables like "%{hp_name}"', f'show tables like "{lp_name}%"', f'show tables like "{ul_name}_"']
    ...
    def checkSuperTableWildcardLength(self):
        ...
        self.cleanTb()
        table_name, hp_name, lp_name, ul_name = self.genTableName()
        tdSql.execute(f"CREATE TABLE {table_name} tags (ts timestamp, a1 int)")
        ...
        exceed_sql_list = [f'show stables like "%{hp_name}"', f'show stables like "{lp_name}%"', f'show stables like "{ul_name}_"']
        for sql in exceed_sql_list:
            tdSql.execute(sql)
            tdSql.checkRows[0]
